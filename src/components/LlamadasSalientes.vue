...
export default {
  ...
  setup() {
    ...
    const llamadaActual = ref({
      ...
      sentido: 'saliente' // Siempre será "entrante"
    });

    const mostrarMensaje = ref(false);

    const guardarLlamada = async (values, { setErrors, resetForm }) => {
      try {
    ...
    const schema = yup.object({
      ...
      subtipo: yup.string().oneOf(['emergencias_sanitarias', 'llamadas_accidentales', 'emergencias_crisis_soledad', 'emergencias_alarma_sin_respuesta', 'notificar_ausencias', 'otros']).required('Subtipo es requerido'),
